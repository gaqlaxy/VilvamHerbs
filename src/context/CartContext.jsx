...
export function CartProvider({ children }) {
  ...
  const addToCart = (product, quantity = 1) => { // Add quantity parameter
    setCart((prev) => {
      const existing = prev.find((item) => item.id === product.id);
      if (existing) {
        return prev.map((item) =>
          item.id === product.id
            ? { ...item, quantity: item.quantity + quantity } // Add instead of increment by 1
            : item
        );
      }
      return [...prev, { ...product, quantity }]; // Use passed quantity
    });
  };
